...
class JSGNN(Encoder):
    ...
    def __init__(self, c, args):
        ...
        for i in range(len(dims) - 1):
            act = acts[i]
            in_dim, out_dim = dims[i], dims[i + 1]
            # only for lineardecoder2 
            if i == (len(dims)-1) - 1 and args.task=='nc':
                if args.dataset not in ['cs', 'amazonphoto']:
                ...
                n_heads = None
            hgc_layers.append(
                hyp_layers.JSGNN_Layer(
                    self.manifold, in_dim, out_dim, c, act, args, n_heads=n_heads, concat_ft=concat_ft
                )
            )
        self.layers = nn.Sequential(*hgc_layers)
        self.encode_graph = True
    ...
    def encode(self, x, adj):
        if self.input_type == 'eucl':
            ...
            x_hyp = x
        return super(JSGNN, self).encode((x_hyp, x), adj)
