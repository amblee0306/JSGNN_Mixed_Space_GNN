...
from layers.hyp_layers import HGATConv, AttentionTemp
...
class LinearDecoder2(Decoder):
    def __init__(self, c, args):
        super(LinearDecoder2, self).__init__(c)
        self.manifold = getattr(manifolds, args.manifold)()
        self.in_features = args.dim
        self.out_features = args.n_classes
        self.c = c
        self.data = args.dataset
        self.decoder_name = 'LinearDecoder2'
        if args.dataset in ['cs', 'amazonphoto']:
            self.bias = args.bias
            self.cls = Linear(self.in_features, self.out_features, args.dropout, lambda x: x, self.bias)

    def forward(self, input):
        x, x_e = input[0]
        if self.data in ['cs', 'amazonphoto']:
            probs = self.cls(x_e)
            return super(LinearDecoder2, self).forward(probs)
        else:
            probs = x_e # already size n_classes
            return super(LinearDecoder2, self).forward(probs)
