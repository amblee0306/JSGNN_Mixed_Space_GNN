...
class HFusion(Module):
    ...
    def forward(self, x_h, x_e):
        ...
        # print('how much euclid to add to hyperbolic', dist)
...
class GATConv(MessagePassing):
    ...
    def forward(self, input, R_dict=None):
        ...
        if R_dict is not None:
            out = self.propagate(edge_index, x=x, num_nodes=x.size(0), R=R)
        else:
        ...
        # RESIDUAL CONNECTION
        if R_dict is not None:
            out = out + x.reshape(out.size()[0], -1)
        else:
            pass

        return out, adj

    def message(self, edge_index_i, x_i, x_j, num_nodes, R_i=None):
        '''
        Constructs messages from node :math:`j` to node :math:`i`
        in analogy to :math:`\phi_{\mathbf{\Theta}}` for each edge in
        :obj:`edge_index`.
        *****
        tensors passed to propagate() can be mapped to the 
        respective nodes  and  by appending _i or _j to the variable name
        '''
        ...
        # TO-DO: IF ALPHA < R, SET ALPHA = 0 BEFORE MULTIPLY X_J
        if R_i is not None:
            alpha = F.relu(alpha - R_i)
            # alpha = torch.where(alpha_tmp > 0, alpha, alpha_tmp) # torch.Size([11684, 8])
